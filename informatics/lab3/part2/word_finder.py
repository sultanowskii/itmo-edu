...
# выполняет задание
REGEX = (
    r'\b[а-я]*'                # есть слово (1), начинающееся на произвольное кол-во любых букв
    r'[' + VWS + r']{2}'       #   2 подряд идущие гласные
    r'[а-я]*\b'                # заканчивается на произвольное кол-во любых букв
    
    r'(?!'                     # и после этого (1) слова нет такого
    r'\s\b[а-я]*'              #   слова, что начинается на произвольное кол-во любых букв
    ...
    r'[а-я]*'                  #     идет гласная
    r'[' + CNS + r']{1}'       #     идет 1 согласная
    r'[а-я]*\b)'               #   и заканчивается на произвольное кол-во любых букв

    r'(?'                      # и после этого (1) слова,
    r'=\s\b[а-я]+)\b'           # вообще есть слово
)
